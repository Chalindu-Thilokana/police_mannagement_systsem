...
<section>
    <div class="flex justify-center items-center min-h-screen p-5 w-full h-full">
        ...
        <form action="{{ isset($user) ? route('branchadmin.update', $user->id) : route('users.store') }}" method="POST" class="py-14">
            @csrf
            ...
            <div class="mt-8">
                <label class="block font-semibold text-black">User branch</label>
                <select name="branch" 
                    class="mt-2 block w-full h-12 rounded-md border border-gray-400 shadow-sm sm:text-sm md:text-base pl-3">
